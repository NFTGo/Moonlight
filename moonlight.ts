...
import { AIRDROP_TARGETS, ERC721_CONTRACT, PRIVATE_KEY, URIS, WEB3_ENDPOINT } from './config';


const erc721Interface: any[] = [
  {
    "inputs": [
      {
        "internalType": "string",
...
class MoonLight {
  ...
  constructor(endpoint: string, contract: string, privateKey: string) {
    ...
    this.contract = new this.web3.eth.Contract(erc721Interface, contract);
  }
   ...
   * @param uris Uri list
   */
  async mintMulti(targets: string[], uris: string[]): Promise<TransactionReceipt> {
...
async function run() {
  const moonlight = new MoonLight(WEB3_ENDPOINT, ERC721_CONTRACT, PRIVATE_KEY);

  await moonlight.mintMulti(AIRDROP_TARGETS, URIS);
}
